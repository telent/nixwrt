...
# To build:
#
# nix-build '<nixpkgs/nixos>' -A vm --arg configuration ./nol.nix

# To run:
#
# sudo SHARED_DIR=`pwd` QEMU_NET_OPTS='hostfwd=tcp::5555-:22' QEMU_OPTS='-nographic -serial mon:stdio -device vfio-pci,host=01:00.0 -m 1024 ' ./result/bin/run-nol-vm
...
let startppp = pkgs.writeScriptBin "start-ppp" ''
  #!${pkgs.bash}/bin/bash
  ${pkgs.rpPPPoE}/bin/pppoe-server -F  -I eth1 -O /etc/pppoe-server-options
'';
